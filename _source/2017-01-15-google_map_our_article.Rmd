...
```{r, echo=FALSE, eval = F}
# save data: I don't want to reveal others data:
save(coord.df.lim, file = "data/coord_df_lim_2.Rda")
```
...
The density is calculated at an exact point through a two-dimensional kernel density. It is estimated on a grid of, in that case, ``r grid_point`` * ``r grid_point`` points.
...
A density of 6 could be read as exp(6) or 403 points per ``r format(round(res_square_meter, 0), big.mark=",", scientific=FALSE)`` square meter or ``r round(exp(6) / (res_square_meter * 1e-4), 0)`` points per hectare.
...
Over ``r format(comp_activity, big.mark=",", scientific=FALSE)`` rows are completed with a list of activity. It represents ``r pct_activity`` of the rows.
...
``r format(nrow(activities_df), big.mark = ",")`` activities are recorded, an average of 7 per location. Nevertheless, we can find a confidence variable. If we restrict to only the highly confident time stamps, over 90, we limit to ``r format(nrow(activities_df_limit), big.mark = ",")`` rows.
